"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UsersAccountsModule = void 0;
const user_entity_1 = require("./domain/user.entity");
const common_1 = require("@nestjs/common");
const users_controller_1 = require("./api/users.controller");
const users_respository_1 = require("./infrastructure/users.respository");
const users_service_1 = require("./application/users.service");
const mongoose_1 = require("@nestjs/mongoose");
const bcrypt_adapter_1 = require("./infrastructure/adapters/bcrypt.adapter");
const users_query_repository_1 = require("./infrastructure/users-query.repository");
let UsersAccountsModule = class UsersAccountsModule {
};
exports.UsersAccountsModule = UsersAccountsModule;
exports.UsersAccountsModule = UsersAccountsModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([{ name: user_entity_1.User.name, schema: user_entity_1.UserSchema }]),
        ],
        controllers: [users_controller_1.UsersController],
        providers: [
            users_respository_1.UsersRepository,
            users_query_repository_1.UsersQueryRepository,
            users_service_1.UsersService,
            bcrypt_adapter_1.BcryptService,
        ],
        exports: [],
    })
], UsersAccountsModule);
//# sourceMappingURL=users-accounts.module.js.map